...
class MID:
    ...
    SetNoRotation = 0x22
    # NEW MODES! Synch settings (added by Vegard)
    # Params
    Synch_SynchOutMode = 0x00
    Synch_SkipFactor = 0x01
    Synch_Offset = 0x02
    Synch_PulseWidth = 0x03
    # Values synchoutmode
    SynchOutMode_Positive_Pulse = 0x0012 # Set synchout to positive pulse mode
    SynchOutMode_Negative_Pulse = 0x0002 # Set synchout to negative pulse mode
    SynchOutMode_Disable = 0x0000 # Disable pulse
    # Values SkipFactor - Unsigned 16-bit value set by user determining how many times the
    # internal sampling instance should be skipped before changing the SynchOut pin state
    SkipFactor_test = 0x00C8 # Skip factor of 200 - 2 Hz sampling

    # Values Offset - unsigned 32 bit value determining the number of clock ticks to delay the synchout
    # state -change after the internal sampling instance. Valid range 0, 513->
    Offset_zero = 0x00000000
    # Values PulseWidth - unsigned 32 bit value that defines the pulse width when synchout mode is set to pulse mode
    # Pulse width is specified in ticks and one tick is equal to 1/29.4912 MHz (internal clock freq). Valid range 0, 1700->
    PulseWidth_1ms = 0x0000733A # 1 millisecond pulse
