...
class MTDevice(object):
    ...
    def __init__(self, port, baudrate=115200, timeout=0.002, autoconf=False,
        ...
        if autoconf:
            ...
            self.settings = None
            # output configurations added by Vegard
            # [[UTC time 20Hz],[SampletimeFine 20Hz], [Euler angles NED 20Hz], [Acceleration 20Hz], [altitude ellipsoid 20Hz],
            # [LatLon 20Hz],[GnssPvtData 4 Hz],[rate of turn 20Hz],[Magnetic field 20 Hz],[Status Word],[BaroPressure 10Hz]]
            self.output_configuration = [[0x1010,0xFFFF],[0x1060,0xFFFF],[0x2034,0x0014],[0x4020,0x0014], [0x5020,0x0014],[0x5040,0x0014],
            [0x7010,0x0004],[0x8020,0x0014],[0xC020,0x0014],[0xE020,0xFFFF], [0x3010,0x000A]]
            #self.output_configuration = [[0x2034,0x0014],[0x4024,0x0014]]
            # sync settings of the MTi (added by Vegard)
            self.sync_settings = [[0x04, 0x04, 0x01, 0x00, 0x0000, 0x007C, 0x0028, 0x0000]]
            # length of the MTData message
            self.length = None
            # header of the MTData message
            self.header = None
            self.configure(self.output_configuration, self.sync_settings)
    ...
    def configure(self, output_configuration, sync_settings, period=None, skipfactor=None):
        """ Configure output and synchronization settings """
        # Function added by Vegard
        self._ensure_config_state()
        self.SetOutputConfiguration(output_configuration)
        self.SetSyncSettings(sync_settings)
        if period is not None:
            self.SetPeriod(period)
        if skipfactor is not None:
            self.SetOutputSkipFactor(skipfactor)
        self.GetConfiguration()
        conf = self.GetOutputConfiguration()
        print("output is: ")
        print(conf)
